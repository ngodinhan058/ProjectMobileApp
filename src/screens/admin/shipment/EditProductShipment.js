...
const EditShippment = ({ navigation, route }) => {
    ...
    const [shipmentDate, setShipmentDate] = useState(new Date(shipmentData?.shipmentDate));
    ...
    const [shipment, setShipment] = useState({
        ...
        shipmentProducts: shipmentData?.shipmentProducts.map(product => ({
            productId: product.productId,
            productPrice: product.shipmentProductPrice.toString(),
            sizesProduct: product.sizesProduct.map(size => ({
                sizeId: size.sizeId,
                productQuantity: size.productQuantity.toString(),
            })),
        })),
    });
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Send data to API
            const response = await axios.put(`${BASE_URL}shipment/${shipmentData.shipmentId}`, formData, {
                headers: {
    ...
    return (
        <>
            ...
            <TouchableOpacity style={styles.button} onPress={handleSubmit} disabled={isLoading}>
                <Text style={styles.buttonText}>Sửa Lô Hàng</Text>
            </TouchableOpacity>
            {isLoading && (
