...
const EditCouponScreen = ({ navigation, route }) => {
    ...
    const {
        ...
        couponExpire: initialCouponExpire,
        couponQuantity: initialCouponQuantity,
        couponPerHundred: initialCouponPerHundred,
        couponPrice: initialCouponPrice,
    ...
    const [couponFeeShip, setCouponFeeShip] = useState(initialCouponFeeShip?.toString() || '');
    ...
    const handleEditCoupon = async () => {
        ...
        try {
            ...
            navigation.replace('CouponList'); // Quay về danh sách coupon
        } catch (error) {
            console.error('Error updating coupon:', error.response?.data || error.message);
            Alert.alert('Error', 'Không thể cập nhật mã giảm giá');
        } finally {
            setIsLoading(false);
        }
    };
            ...
            case COUPON_SHIP_TYPE:
                return (
                    <>
                        ...
                        <TextInput
                            style={styles.input}
                            placeholder="Nhập số tiền giảm giá vận chuyển"
                            keyboardType="numeric"
                            value={couponFeeShip}
        ...
        <View style={styles.container}>
            <ScrollView>
                <LinearGradient colors={['#2196F3', '#1976D2']} style={styles.header}>
                    ...
                    <Text style={styles.textHeader}>Chỉnh Sửa Mã Giảm Giá</Text>
                </LinearGradient>
